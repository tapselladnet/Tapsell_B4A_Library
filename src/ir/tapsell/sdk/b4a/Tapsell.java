...
@BA.Events(values = {"onAdShowFinished (zoneId As String, adId As String, completed As Boolean, rewarded As Boolean)",
        "onAdAvailable (zoneId As String, adId As String)",
        "onNoAdAvailable (zoneId As String)",
        "onNoNetwork (zoneId As String)",
        "onExpiring (zoneId As String, adId As String)",
        "onOpened (zoneId As String, adId As String)",
        "onClosed (zoneId As String, adId As String)",
        "onError(zoneId As String, error As String)",
		"onNativeBannerAdAvailable (zoneId As String, adId As String)",
		"onNoNativeBannerAdAvailable (zoneId As String)",
		"onNativeVideoAdAvailable (zoneId As String, adId As String)",
		"onNoNativeVideoAdAvailable (zoneId As String)",
		"onNoBannerAdAvailable (zoneId As String)",
		"onBannerNoNetwork (zoneId As String)",
		"onBannerAdRequestFilled (zoneId As String)",
		"onBannerAdMadeHidden (zoneId As String)",
		"onBannerAdError (error As String , zoneId As String)"
        })
public class Tapsell implements TapsellB4AListener {
    ...
    private static final Map<String, TapsellBannerView> bannerList = Collections.synchronizedMap(new WeakHashMap<String, TapsellBannerView>());    
    ...
    public boolean initialize( BA ba,String appKey)
    {
        ...
        return TapsellB4A.initialize(ba.activity,appKey);
    }
    ...
    public void showAd(String id, boolean back_disabled, boolean immersive_mode, int rotation_mode, boolean show_dialog)
    {
    	if(mBA==null)
    	{
    		return ;
			...
			public void run() {
				if(mBA!=null)
		        {
		    		if(mBA.subExists("tapsell_onadshowfinished"))
		    		{
		    			...
		    			Log.e("B4A","onAdShowFinished, zoneId: "+mZoneId+", add: "+adId+", completed? "+completed+", rewarded? "+rewarded);
		    			mBA.raiseEvent2(mBA,true,"tapsell_onadshowfinished",false,mZoneId,adId,completed,rewarded);
		    		}
		        }
			}
		});
	...
	public void fillNativeBannerAd(final String zoneId,
                            ...
                            ViewGroup adContainer)
	{
		...
		TapsellNativeBannerAdLoader.fillNativeBannerAd(mBA.activity, zoneId,
				tvTitle, tvDescription,
				ivBanner, ivLogo,
				tvCTAButton, tvSponsored,
				adContainer, new TapsellNativeBannerAdLoadListener() {
	...
	public void fillNativeVideoAd(final String zoneId,
            boolean autoStartVideo,
            boolean muteVideo, boolean muteVideoBtnEnabled,
            TextView tvTitle, TextView tvDescription,
            ViewGroup videoView, ImageView ivLogo,
            TextView tvCTAButton, TextView tvSponsored,
		...
		TapsellNativeVideoAdLoader.fillNativeVideoAd(mBA.activity, zoneId,
				autoStartVideo,false,muteVideo,muteVideoBtnEnabled,
				tvTitle, tvDescription,
				videoView, ivLogo,
				tvCTAButton, tvSponsored,
				parentView,new TapsellNativeVideoAdLoadListener() {
	...
	{
		TapsellBannerView bannerAdView = new TapsellBannerView(mBA.activity, TapsellBannerType.fromValue(bannerType),zoneId);
		...
		adContainer.addView(bannerAdView, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
